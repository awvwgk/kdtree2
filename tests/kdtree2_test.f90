...
program kd_tree_test
  ...
  real(kdkind) :: r2array(5) = [1.0e-4,1.0e-3,5.0e-3,1.0e-2,2.0e-2]
  integer   :: nnarray(5) = [1, 5, 10, 25, 500]
  ...
  do k=1,50
     ...
     call kdtree2_n_nearest_brute_force(tp=tree3, qv=query_vec, &
          nn=nnbrute, results=resultsb)

     call kdtree2_n_nearest_around_point(tp=tree3,idxin=rind,correltime=-1, &
          nn=nnbrute, results=results)
